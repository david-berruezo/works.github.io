...
ContentManager.AUTOMATICALLY_TRACK_GOOGLE_ANALYTICS = !0;
ContentManager.SHOW_TRACES = !1;
...
ContentManager.init = function(xml, defaultPath) {
    ...
    defaultPath && (ContentManager._defaultPath = defaultPath, ContentManager.trace(ContentManager._defaultPath, "_defaultPath"));
    //window.addEventListener("hashchange", ContentManager.onHashChange);
    //window.addEventListener("hashchange", ContentManager.onHashChange);
    document.addEventListener("DOMContentLoaded", ContentManager.on_document_load(event));
    "Explorer" == BrowserDetect.BROWSER_NAME && BrowserDetect.BROWSER_VERSION <= 7 && ContentManager.autoCheck();
    //ContentManager.onHashChange();
};

ContentManager.path = function(newPath) {
    var getType = typeof newPath;
    "object" == getType && (newPath = ContentManager.composeFullPathFromXML(newPath));
...
ContentManager.nextTemplate = function(passedVars) {
    ...
    ContentManager._blocked = !1;
    //ContentManager.onHashChange();
    ContentManager.on_document_load();
};
...
ContentManager.onHashChange = function() {
    ...
    if (ContentManager.trace("-----------------------------"), 0 == ContentManager._blocked) {
        ...
        for ("" == pathArr[0] && (pathArr[0] = ContentManager._defaultPath), ContentManager.trace(pathArr, "pathArr"); pathArr.length > 0;) 
        pathXML = ContentManager.findContent(pathArr),
        pathXML && (templateName = pathXML.getAttribute("data-template"), 
        templateName && ContentManager.findTemplateFromName(templateName) && (templateLevel = ContentManager.findTemplateLevelFromName(templateName), 
        templateLevel === !1 && (templateLevel = pathArr.length - 1), 
        ...
        if (ContentManager.trace(type, "type"), "none" != type && candidate)
            ...
            if (JSClass) {
                ...
                ContentManager._prevTemplateObj && (templateData.setPrevTemplateData(ContentManager._prevTemplateObj.templateData), ContentManager._prevTemplateObj = null), 
                ContentManager._passedVariables && (templateData.setPassedVariables(ContentManager._passedVariables), ContentManager._passedVariables = null);
...
ContentManager.on_document_load = function(){
    // 5 == section | 6 == project/project | 4 == home
    var my_url = window.location.pathname;
    var levels = my_url.split("/");
    var parametro = "'#/studio/'";
    if (levels.length == 2){
    //if (levels.length == 5){
        parametro = '#'+levels[levels.length-1];
    }else if(levels.length == 3){
    //}else if(levels.length == 6){
        parametro = '#'+levels[levels.length-2]+'/'+levels[levels.length-1];
    }else{
        parametro = '#home';
    }
    if (ContentManager.trace("-----------------------------"), 0 == ContentManager._blocked) {
        if (ContentManager._newPath = ContentManager.extractPath(parametro), ContentManager._oldPath === ContentManager._newPath) {
        //if (ContentManager._newPath = ContentManager.extractPath(window.location.hash), ContentManager._oldPath === ContentManager._newPath) {
            if (ContentManager._samePathCount += 1, ContentManager._samePathCount > 3) return void console.error("ContentManager.as bug, same path is repeated! - this is a show breaker :(")
        } else ContentManager._samePathCount = 0;
        ContentManager._oldPath = ContentManager._newPath;
...
ContentManager.extractPath = function(str) {
    var i, currPart, arr1 = str.split("#"),arr2 = arr1[arr1.length - 1].split("/"),arr3 = new Array,l = arr2.length;
...
ContentManager.findTemplateFromName = function(templateName) {
    var i, regObj, JSClass, register = ContentManager._templateRegister,l = register.length;
    for (i = 0; l > i; i += 1)
...
ContentManager.getChildByAttr = function(xml, attr, value) {
    var i, child, children = xml.children, l = children.length, rVal = null;
    for (i = 0; l > i; i += 1)
...
ContentManager.getChildsByAttr = function(xml, attr, value){
    var i, child, children = xml.children, l = children.length, rVal = [];
    for (i = 0; l > i; i += 1)
        if (child = children[i], child.getAttribute("data-" + attr) == value) {
            rVal.push(child);
        } return rVal
...
ContentManager.getAllChildrenByBackwardsWithAttribute = function(target, name) {
    for (var parent = target, children = [], currAttr = null; null !== parent;) 
    "function" == typeof parent.getAttribute && (currAttr = parent.getAttribute("data-" + name), currAttr && children.unshift(parent)), 
