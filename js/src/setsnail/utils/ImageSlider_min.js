function ImageSlider(urls) {
    ...
    function setupImages() {
        for (var l = urls.length, xPos = 0, i = 0; l > i; i++) {
            ...
            "mp4" == type ? (console.log("SÆLKDFJLKSDJFLKSJDFLKDJS"), img = new VideoPlayer(urls[i]), img.init("AUTOPLAY"), img.setScaleMode("OUTSIDE"), img.enableLoop(), img.mute(), img.play()) : (img = new RetinaImage(urls[i]), img.init(), img.setPreloader(new SlidePreloader("#f4f4f4", "#e9e9e9")), img.setResizeMode("insideBox"), img.setPosition("center/center")), img.setSize(_width, _height), TweenMax.set(img, {
                x: xPos
