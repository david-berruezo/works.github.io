function GuideLines(width) {
    ...
    return _instance.addGuide = function(name, offset) {
        _guides[name] = {
            offset: offset,
            orig: offset
        }
    ...
    _instance.setRatioToAll = function(ratio) {
        for (var key in _guides) _instance.setRatioTo(key, ratio)
    },

    _instance.getDistance = function(name1, name2) {
        var guide1 = _instance.getGuide(name1),
            guide2 = _instance.getGuide(name2);
    ...
    _instance.getGuide = function(name) {
        ...
        return guide || console.error("guide '" + name + "' was not found"), guide.offset
    },
